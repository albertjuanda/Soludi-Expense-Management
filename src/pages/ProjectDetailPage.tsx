...
const ProjectDetailPage: React.FC = () => {
  ...
  if (!project) return (
    <div className="flex flex-col items-center justify-center py-24">
      ...
      <button onClick={() => navigate('/projects')} className="mt-4 text-indigo-600 font-semibold text-sm">← Back to Projects</button>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          ...
          <span className={`text-xs font-semibold px-3 py-1 rounded-full flex-shrink-0 ${status.classes}`}>{status.label}</span>
        </div>
        <div className="flex flex-wrap gap-6 text-sm text-slate-500">
          ...
          {admin && (
            <div className="flex items-center gap-2">
              <Users size={14} />
              <span>Admin: <span className="font-semibold text-slate-700">{admin.name}</span></span>
            </div>
          )}
          {project.budget && (
      ...
      {/* Financial overview */}
      <div>
        <h2 className="text-base font-bold text-slate-800 mb-3">Financial Overview</h2>
        <HelicopterView expenses={projectExpenses} budget={project.budget} />
      </div>

      {/* Team */}
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        <h2 className="text-sm font-bold text-slate-800 mb-4">Assigned Team ({assignedUsers.length})</h2>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-bold text-slate-800">Expense Requests ({projectExpenses.length})</h2>
        </div>
        {projectExpenses.length === 0 ? (
          <EmptyState
            icon={<FileText size={28} className="text-slate-300" />}
            title="No expenses yet"
            description="No expense requests have been submitted for this project."
          />
        ) : (
