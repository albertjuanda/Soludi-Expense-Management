  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Manage expense categories and reimbursement schedules.</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-slate-800">Expense Categories</h2>
          <span className="ml-auto text-xs font-semibold text-slate-400">{categories.length} categories</span>
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-base font-bold text-slate-800">Reimbursement Schedule</h2>
        </div>
        <BulkReimbursement />
      </div>
      ...
      <Modal isOpen={!!editCat} onClose={() => setEditCat(null)} title="Edit Category">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => setEditCat(null)} className="px-4 py-2 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl">Cancel</button>
            <button onClick={handleEditCategory} className="px-4 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl">Save</button>
          </div>
        </div>
      </Modal>
        ...
        <div className="space-y-4">
          <div className="flex gap-3 p-4 bg-rose-50 border border-rose-200 rounded-xl">
            ...
            <p className="text-sm text-rose-700">
              Are you sure you want to delete <span className="font-bold">{catToDelete?.name}</span>? This cannot be undone.
            </p>
          </div>
          <div className="flex justify-end gap-3">
            <button onClick={() => setDeleteCat(null)} className="px-4 py-2 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl">Cancel</button>
